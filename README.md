...
## Approach
The approach taken was to quickly and simply design a solution to the details provided, while capturing any assumptions and questions to feed future iterations of the design.  Some basic tenets utilized:
* Solve the problem in the most rudimentary and simplistic way that is easily maintainable and direct
* Solve the problem first and foremost, while capturing a few assumptions and questions to feed future iterations, if necessary
...
* The input string is a literal string and data types or data values are not evaluated.  Adjust the solution if the input string is a representation of a data set and types or values need to be accommodated.
...
* Basic error handling was added for demonstration purposes.  We would want to determine use and need based on expected input/output.
* Internal was chosen for the 2 classes defined.  If this project is extended or used elsewhere, this should be re-evaluated.

## Questions
* What is the overall problem that this exercise is intended to solve?
...
* Is there a desired output location? If so, are there multiple?
